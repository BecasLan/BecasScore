...
export class DashboardAPI {
  ...
  /**
   * GET /api/user/:id
   * Returns trust score and basic user info
   */
  private async getUserScore(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Get trust score (use first guild the user is in, or 'global' as fallback)
      const guildId = this.client.guilds.cache.first()?.id || 'global';
      const trustScoreObj = this.trustEngine.getTrustScore(userId, guildId);
      const trustScore = trustScoreObj.score; // Already in 0-100 range

      console.log(`📊 API getUserScore: userId=${userId}, guildId=${guildId}, score=${trustScore}`);
