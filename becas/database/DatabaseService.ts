...
export class DatabaseService {
  ...
  constructor() {
    ...
    // Try to initialize Supabase as fallback
    try {
      this.supabase = getSupabaseClient();
      logger.info('✅ Supabase REST API available as fallback');
    } catch (error) {
      logger.warn('Supabase REST API not configured, using PostgreSQL only');
    }
  }
