...
async function main() {
  ...
  // 🚀 Initialize Kernel Architecture (NEW!)
  logger.info('');
  logger.info('Initializing Kernel Architecture...');
  const kernelBootstrap = new KernelBootstrap({ client });

  try {
    await kernelBootstrap.initialize({
      ollamaService: becas.getOllamaService(),
    });
    logger.info('✓ Kernel architecture initialized successfully');
  } catch (error) {
    logger.error('Failed to initialize kernel architecture', error);
    console.error('⚠️  Kernel architecture initialization failed (non-fatal):', error);
    // Don't exit - kernel is optional, existing BecasCore still works
  }
  ...
  process.on('SIGINT', async () => {
    ...
    try {
      ...
      if (kernelBootstrap) {
        ...
        console.log('🔧 Shutting down kernel architecture...');
        await kernelBootstrap.shutdown();
      }
