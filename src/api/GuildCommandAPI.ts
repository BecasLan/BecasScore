...
export class GuildCommandAPI {
  ...
  private async getGuildStats(req: Request, res: Response): Promise<void> {
    try {
      ...
      const trustScores = Array.from(members.values()).map(member => {
        const data = this.trustEngine.getTrustScore(member.id, guild.id);
  ...
  private async handleBanIntent(
    ...
    const trustData = this.trustEngine.getTrustScore(userId, guild.id);
  ...
  private async handleTrustScoreQuery(
    ...
    if (thresholdMatch) {
      ...
      const lowTrustUsers = Array.from(members.values())
        .map(member => ({
          id: member.id,
          username: member.user.username,
          trustData: this.trustEngine.getTrustScore(member.id, guild.id)
        }))
    ...
    const members = await guild.members.fetch();
    const trustScores = Array.from(members.values()).map(m =>
      this.trustEngine.getTrustScore(m.id, guild.id).score
    );
