...
- involvedUsers: array of usernames involved in tension`;
  ...
  "involvedUsers": string[]
      ...
      // 🔥 FIX: Handle empty/invalid JSON from Ollama
      if (!result || !result.riskLevel) {
        console.warn('⚠️ ConflictPredictor: Invalid JSON from Ollama, returning safe default');
        return {
          riskLevel: 'none',
          confidence: 0,
          indicators: [],
          reasoning: 'AI analysis failed - returning safe default',
