...
export class TrustScoreEngine {
  ...
  private databaseService?: DatabaseService; // Supabase connection

  constructor(storage: StorageService, auditLogger?: AuditLogger, databaseService?: DatabaseService) {
    ...
    if (data?.scores) {
      Object.entries(data.scores).forEach(([key, score]) => {
        ...
        const deserializedScore: TrustScore = {
          ...score,
          score: score.score ?? TRUST_CONFIG.DEFAULT_SCORE, // FIX: Default to 50 if undefined
  ...
  async modifyTrust(
    ...
    // TODO: Implement Supabase sync via DatabaseService
    // For now, use TrustScoreEngineDB for database operations
  ...
  async checkRedemption(userId: string, guildId: string, message: {
    ...
    if (this.databaseService) {
      try {
        const sicilRepo = new SicilRepository(this.databaseService);
        const sicil = await sicilRepo.getSicilSummary(guildId, userId);

        if (sicil && sicil.scam_violations > 0) {
          console.log(`🚫 Redemption BLOCKED for ${userId}: ${sicil.scam_violations} scam violations in history`);
          return {
            redeemed: false,
            points: 0,
        ...
        console.warn('Failed to check scam history for redemption:', err);
      }
    }
