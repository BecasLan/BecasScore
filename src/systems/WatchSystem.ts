...
export class WatchSystem {
  ...
  private async isUserWatched(userId: string, watch: WatchConfig, guild: Guild): Promise<boolean> {
    ...
    if (watch.filter) {
      ...
      if (watch.filter.trustScoreMin !== undefined || watch.filter.trustScoreMax !== undefined) {
        const trustScore = await this.trustEngine.getTrustScore(userId, guild.id);
  ...
  private async checkCondition(
    ...
    switch (condition.type) {
      ...
      case 'trust_drop':
        return await this.checkTrustDrop(message, condition.threshold || 50);
...
{"sentiment": "positive" | "negative" | "neutral", "confidence": 0.0-1.0, "reason": "why"}`;
   ...
   */
  private async checkTrustDrop(message: Message, threshold: number): Promise<{ triggered: boolean; evidence: string; confidence: number }> {
...
Return JSON: {"sentiment": number between -1 and 1}`;
  ...
  private async evaluateActionCondition(
    ...
    switch (condition.type) {
      case 'trust_score':
        actualValue = (await this.trustEngine.getTrustScore(event.userId, event.message.guild!.id)).score;
        break;
