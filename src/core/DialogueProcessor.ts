...
export class DialogueProcessor {
  ...
  async analyzeMessage(message: MessageContext): Promise<AnalyzedMessage> {
    // 🧠 PARALLEL GPU ANALYSIS: All 3 analyses run simultaneously
    try {
      const [sentiment, toxicity, intent] = await Promise.all([
        this.analyzer.analyzeSentiment(message.content),
        this.analyzer.detectToxicity(message.content),
        this.analyzer.extractIntent(message.content),
      ]);
